...
public class EquipmentService : IEquipmentsService
{
    private readonly IEquipmentsRepository _entitiesRepository;
    ...
    public EquipmentService(IEquipmentsRepository entitiesRepository, IMapper mapper, JwtHelper jwtHelper, ITransactionsRepository transactionsRepository, ITransactionTypesRepository transactionTypesRepository, ILogger<EquipmentService> logger)
    {
    ...
    public async Task<EntityGetResponse> GetAsync(int? id, int? invNumber)
    {
        ...
        {
            var entity = _entitiesRepository.GetAllByPredicate(e => e.InvNumber == invNumber).FirstOrDefault() ?? throw new ModelNotFoundException("Equipment with given invNumber not found");
            return _mapper.Map<EntityGetResponse>(entity);
        }
    ...
    public async Task<EntityGetResponse> ReserveAsync(int? id, int? invNumber, string jwt)
    {
        ...
        {
            _logger.LogWarning("{nameof(Equipment)} with id={id} is already reserved", nameof(Equipment), id);
            throw new EntityReservationException("Equipment is already reserved");
        }
    ...
    public async Task<EntityGetResponse> UnreserveAsync(int? id, int? invNumber, string jwt)
    {
        ...
        {
            _logger.LogWarning("Can't unreserve {nameof(Equipment)} with id={id}, because it's reserved by another employee or not reserved at all", nameof(Equipment), id);
    ...
    { 
        ...
        {
            _logger.LogWarning("Can't update {nameof(Equipment)} with id={id}, because {nameof(Equipment)} with invNumber={invNumber} already exists", nameof(Equipment), id, nameof(Equipment), request.InvNumber);
    ...
    public async Task<EntityCreateResponse> CreateAsync(EntityCreateRequest request, string jwt)
    {
        ...
        {
            _logger.LogWarning("Can't create {nameof(Equipment)}, because invNumber={invNumber} already exists", nameof(Equipment), request.InvNumber);
            throw new InvNumberAlreadyExistsException("InvNumber already exists");
        }
        var trType = _transactionTypesRepository.GetAllByPredicate(t => t.Type.ToLower() == "create").First();
        var mappedEntity = _mapper.Map<Equipment>(request);
